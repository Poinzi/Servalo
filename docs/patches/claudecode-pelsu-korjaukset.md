# Pelastussuunnitelma-työkalu — korjaukset (täysi tiedosto)

Tämä prompti luo / korvaa **itsenäisen** tiedoston `pelastussuunnitelma-tyokalu.html`
kansiossa `~/Desktop/Servalo`. Tiedosto ei ole osa deployattua Servalo-sovellusta
(servalo.html / ohje24.html / server.js / api.js) — se on erillinen selaimessa avattava työkalu.

Korjaukset tässä versiossa:
1. **Mobiilisyöttö:** Riskiarvion Uhka ja Paino syötetään isoilla 0–5-napeilla (kortti per rivi) numerokentän sijaan.
2. **Y-tunnus:** oma kenttä perustietoihin, näkyy etusivulla ja tiivistelmässä.
3. **Kuvat:** etusivun ulkokuva + liitekuvat (asemapiirros, poistumistiekartta…), pakataan selaimessa.
4. **Tulostuksen sivutus:** otsikot pysyvät sisällön kanssa, lohkot eivät katkea, taulukon rivit ehjinä.
5. **Alatunniste:** oma toistuva alatunniste jokaiselle tulostetulle sivulle.

Työskentele kansiossa `~/Desktop/Servalo`. Aja TÄSMÄLLEEN alla olevat komennot.
Jos jokin validointi epäonnistuu, PYSÄHDY ja raportoi — älä arvaa.
      ...
      <div class="panel-b">
        <div class="field" style="margin-bottom:14px">
          ...
          <div class="imgrow">
            ...
            <div>
              <button class="btn sm" onclick="document.getElementById('coverFile').click()">📷 Valitse kuva</button>
        ...
        <div class="field">
          <label>Liitekuvat (asemapiirros, poistumistiekartta, kokoontumispaikka…)</label>
          <div>
            <button class="btn sm" onclick="document.getElementById('liiteFile').click()">➕ Lisää liitekuva</button>
...
function normM(){if(!M.kohde)M.kohde={};if(!Array.isArray(M.rows)||!M.rows.length)initRows();if(!Array.isArray(M.liitteet))M.liitteet=[];if(typeof M.kansi!=="string")M.kansi=M.kansi||"";}
...
function pickCover(input){
  ...
  resizeImage(f,1000,0.8,function(url){
    M.kansi=url;
...
function pickLiite(input){
  var f=input.files&&input.files[0];input.value="";if(!f)return;
  resizeImage(f,1200,0.8,function(url){
...
function renderRows(){
  ...
  b.innerHTML=M.rows.map(function(r,i){
    ...
    return '<div class="rcard" id="rc'+i+'">'+
      '<div class="rc-top">'+
        '<span class="cat">'+esc(r.aihe)+'</span>'+
      ...
      '</div>'+
      '<div class="rc-desc"><b>'+esc(r.kuvaus)+'.</b> '+esc(r.toimet)+'</div>'+
...
function renderPlan(){
  ...
  html+='<h3>Karttapiirros</h3><ul><li>Suunnitelman liitteenä toimitetaan pohjakartta, johon merkitään poistumisreitit, sammutuskalusto, ensiapupisteet, väestönsuoja sekä sähkön, kaasun ja veden pääsulut ja kokoontumispaikka</li></ul>';
  ...
  html+='<div class="note">Tämä pelastussuunnitelma on laadittu pelastuslain 379/2011 mukaisesti. Tarkista aina paikallisen pelastusviranomaisen ajantasaiset ohjeet. Tulostettu '+(new Date().toLocaleDateString("fi-FI"))+'.</div>';
...
function clearAll(){
  ...
  M={kohde:{},rows:[],liitteet:[],kansi:""};initRows();save();location.reload();
}

// init
if(!load()){M={kohde:{},rows:[],liitteet:[],kansi:""};initRows();}
normM();
bindKohde();renderRows();renderSummary();renderCover();renderLiitteet();
</script>
</body>
</html>
PELSU_EOF
echo "Tiedosto kirjoitettu: pelastussuunnitelma-tyokalu.html"
```

---

## 2) VALIDOINTI (aja ennen committia)

```bash
cd ~/Desktop/Servalo

# a) HTML:n JS-syntaksi kunnossa (pois lukien JSON-datablokki)
node -e '
const fs=require("fs");
const h=fs.readFileSync("pelastussuunnitelma-tyokalu.html","utf8");
const blocks=[...h.matchAll(/<script(?![^>]*application\/json)[^>]*>([\s\S]*?)<\/script>/g)].map(m=>m[1]).join("\n");
new Function(blocks);
console.log("JS OK");
const rows=JSON.parse(h.match(/id="riskdata"[^>]*>([\s\S]*?)<\/script>/)[1]);
if(rows.length!==52){console.error("VAROITUS: riskirivejä "+rows.length+" (odotus 52)");process.exit(1);}
console.log("riskdata rows="+rows.length);
' || exit 1

# b) uudet ominaisuudet löytyvät tiedostosta
for s in "segHtml" "setScore" "class=\"segb" "data-k=\"ytunnus\"" "pickCover" "pickLiite" "resizeImage" "printfoot" "table-header-group" "page-break-after:avoid"; do
  grep -q "$s" pelastussuunnitelma-tyokalu.html && echo "OK: $s" || { echo "PUUTTUU: $s"; exit 1; }
done

# c) ei kiellettyjä merkkijonoja
if grep -rnE "PAP Group|idrcloud|Prosero|Kramp|0929411|@pap\.fi|jari\.mattila@savuks\.fi|0443665638" pelastussuunnitelma-tyokalu.html; then
  echo "VAROITUS: tarkista osumat"; else echo "Ei kiellettyjä merkkijonoja"; fi
```

Kaikki `OK:` + `JS OK` + `Ei kiellettyjä merkkijonoja` → jatka. Muuten pysähdy.

---

## 3) (Valinnainen) COMMIT & PUSH

Aja vain jos haluat tiedoston mukaan git-repoon. Tiedosto on staattinen eikä vaikuta
palvelimen toimintaan, mutta se ei myöskään ole pakollinen osa deploytä.

```bash
cd ~/Desktop/Servalo
rm -f .git/index.lock
git add pelastussuunnitelma-tyokalu.html
git commit -m "Pelastussuunnitelma-tyokalu: mobiili 0-5-napit, Y-tunnus, kuvat, tulostuksen sivutus ja alatunniste"
...
1. Avaa `pelastussuunnitelma-tyokalu.html` selaimessa (tai kännykällä).
2. Välilehti 2 (Riskiarvio): napauta Uhka/Paino 0–5 — TOT päivittyy heti.
3. Välilehti 1: syötä Y-tunnus, lisää etusivun kuva ja liitekuva.
4. Välilehti 3 → 🖨 Tulosta / PDF. Poista selaimen tulostusikkunasta rasti
   "Ylä- ja alatunnisteet", niin jäljelle jää oma alatunniste. Tarkista että
   otsikot eivät jää yksin sivun pohjalle.
